...
def anonymize_video(
    ...
    **kwargs,
    ...
    def process_frame(frame, idx):
        ...
        frame_torch = utils.im2torch(frame, to_float=False, normalize=False)[0]

        cache_id_ = hashlib.md5(frame).hexdigest() + str(idx)
        synthesis_kwargs["cache_id"] = cache_id_
        if visualize_detection:
            anonymized = anonymizer.visualize_detection(frame_torch, str(idx), cache_id=cache_id_)
        else:
            anonymized = anonymizer(frame_torch, str(idx), **synthesis_kwargs)
